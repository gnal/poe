...
class CrawlCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->weaponTypeManager = $this->getContainer()->get('poe_core.weapon_manager');
        $this->armorTypeManager = $this->getContainer()->get('poe_core.armor_manager');
    ...
    protected function process($page = 1)
    {
        ...
        foreach ($hrefs as $href) {
            ...
            // we delete all items with this thread id so we dont have duplicates if we crawl same threads
            $dql = 'delete from '.$this->weaponManager->getClass().' i where i.threadId = '.$threadId;
            $numDeleted = $this->weaponManager->getEntityManager()->createQuery($dql)->execute();
            $this->output->writeln("<error>DELETED</error> ".$numDeleted." for thread #".$threadId);

            $dql = 'delete from '.$this->ArmorManager->getClass().' i where i.threadId = '.$threadId;
            $numDeleted = $this->ArmorManager->getEntityManager()->createQuery($dql)->execute();
            $this->output->writeln("<error>DELETED</error> ".$numDeleted." for thread #".$threadId);
            ...
            foreach ($data as $v) {
                ...
                if (1) {
                    $this->createWeapon($row, $i);
                } elseif (2) {
                    $this->createArmor($row, $i);
                }
    ...
    private function createWeapon($data, $i)
    {
        $item = $this->weaponManager->create();

        $this->setExplicitMods($item);

        $this->weaponManager->updateBatch($item, $i);
    }

    private function createArmor($data, $i)
    {
        $item = $this->armorManager->create();

        $this->setExplicitMods($item);

        $this->armorManager->updateBatch($item, $i);
    }
    ...
    private function setExplicitMods($item)
    {
